use {
    crate::helpers::suite::{
        core::{extension::send_tx, App, ProgramId},
        types::AppUser,
    },
    anchor_lang::{prelude::*, Result},
    litesvm::types::TransactionMetadata,
    solana_program::system_instruction,
    spl_associated_token_account::instruction as ata_instruction,
    spl_token::instruction as token_instruction,
};

pub const WSOL_MINT: Pubkey = spl_token::native_mint::id();

pub trait WsolExtension {
    /// create a wrapped SOL account for a user
    fn wsol_create_account(
        &mut self,
        user: AppUser,
        amount_lamports: u64,
    ) -> Result<TransactionMetadata>;

    /// wrap SOL - convert native SOL to wrapped SOL tokens
    fn wsol_wrap(&mut self, user: AppUser, amount_lamports: u64) -> Result<TransactionMetadata>;

    /// unwrap SOL - convert wrapped SOL tokens back to native SOL
    fn wsol_unwrap(&mut self, user: AppUser) -> Result<TransactionMetadata>;
}

impl WsolExtension for App {
    fn wsol_create_account(
        &mut self,
        user: AppUser,
        amount_lamports: u64,
    ) -> Result<TransactionMetadata> {
        let ProgramId { token_program, .. } = self.program_id;

        let payer = user.pubkey();
        let signers = [user.keypair()];

        // ata
        let wrapped_sol_ata = App::get_ata(&payer, &WSOL_MINT);

        // create the account
        let create_ata_ix = ata_instruction::create_associated_token_account(
            &payer,
            &payer,
            &WSOL_MINT,
            &token_program,
        );

        // transfer SOL
        let transfer_ix = system_instruction::transfer(&payer, &wrapped_sol_ata, amount_lamports);

        // sync native instruction to convert SOL to wrapped SOL
        let sync_native_ix = token_instruction::sync_native(&token_program, &wrapped_sol_ata)?;

        send_tx(
            &mut self.litesvm,
            &[create_ata_ix, transfer_ix, sync_native_ix],
            &payer,
            &signers,
        )
    }

    fn wsol_wrap(&mut self, user: AppUser, amount_lamports: u64) -> Result<TransactionMetadata> {
        let ProgramId { token_program, .. } = self.program_id;

        let payer = user.pubkey();
        let signers = [user.keypair()];

        // ata
        let wrapped_sol_ata = App::get_ata(&payer, &WSOL_MINT);

        // transfer SOL
        let transfer_ix = system_instruction::transfer(&payer, &wrapped_sol_ata, amount_lamports);

        // sync native instruction to convert SOL to wrapped SOL
        let sync_native_ix = token_instruction::sync_native(&token_program, &wrapped_sol_ata)?;

        send_tx(
            &mut self.litesvm,
            &[transfer_ix, sync_native_ix],
            &payer,
            &signers,
        )
    }

    fn wsol_unwrap(&mut self, user: AppUser) -> Result<TransactionMetadata> {
        let ProgramId { token_program, .. } = self.program_id;

        let payer = user.pubkey();
        let signers = [user.keypair()];

        // ata
        let wrapped_sol_ata = App::get_ata(&payer, &WSOL_MINT);

        // close the account instruction - this unwraps the SOL
        let close_account_ix = token_instruction::close_account(
            &token_program,
            &wrapped_sol_ata,
            &payer,
            &payer,
            &[],
        )?;

        send_tx(&mut self.litesvm, &[close_account_ix], &payer, &signers)
    }
}
